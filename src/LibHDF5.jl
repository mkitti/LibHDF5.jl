...
function Base.getproperty(x::Ptr{H5F_info2_t}, f::Symbol)
    f === :super && return Ptr{var"##Ctag#326"}(x + 0)
    f === :free && return Ptr{var"##Ctag#327"}(x + 24)
    f === :sohm && return Ptr{var"##Ctag#328"}(x + 48)
    return getfield(x, f)
end
...
function Base.getproperty(x::Ptr{H5O_hdr_info_t}, f::Symbol)
    ...
    f === :space && return Ptr{var"##Ctag#319"}(x + 16)
    f === :mesg && return Ptr{var"##Ctag#320"}(x + 48)
    return getfield(x, f)
end
...
function Base.getproperty(x::Ptr{H5O_native_info_t}, f::Symbol)
    ...
    f === :meta_size && return Ptr{var"##Ctag#318"}(x + 64)
    return getfield(x, f)
end
...
function Base.getproperty(x::Ptr{H5O_info1_t}, f::Symbol)
    ...
    f === :meta_size && return Ptr{var"##Ctag#323"}(x + 128)
    return getfield(x, f)
end
...
function Base.getproperty(x::Ptr{H5F_info1_t}, f::Symbol)
    ...
    f === :sohm && return Ptr{var"##Ctag#322"}(x + 8)
    return getfield(x, f)
end
...
function Base.getproperty(x::Ptr{H5VL_loc_params_t}, f::Symbol)
    ...
    f === :loc_data && return Ptr{var"##Ctag#329"}(x + 8)
    return getfield(x, f)
end
...
const ULLONG_MAX = typemax(Clonglong)
...
using Libdl
H5_load_global(sym) = unsafe_load(Ptr{hid_t}(Libdl.dlsym(HDF5_jll.libhdf5_handle, sym)))

const H5T_IEEE_F32BE = H5_load_global(:H5T_IEEE_F32BE_g)

const H5T_IEEE_F32LE = H5_load_global(:H5T_IEEE_F32LE_g)

const H5T_IEEE_F64BE = H5_load_global(:H5T_IEEE_F64BE_g)

const H5T_IEEE_F64LE = H5_load_global(:H5T_IEEE_F64LE_g)

const H5T_STD_I8BE = H5_load_global(:H5T_STD_I8BE_g)

const H5T_STD_I8LE = H5_load_global(:H5T_STD_I8LE_g)

const H5T_STD_I16BE = H5_load_global(:H5T_STD_I16BE_g)

const H5T_STD_I16LE = H5_load_global(:H5T_STD_I16LE_g)

const H5T_STD_I32BE = H5_load_global(:H5T_STD_I32BE_g)

const H5T_STD_I32LE = H5_load_global(:H5T_STD_I32LE_g)

const H5T_STD_I64BE = H5_load_global(:H5T_STD_I64BE_g)

const H5T_STD_I64LE = H5_load_global(:H5T_STD_I64LE_g)

const H5T_STD_U8BE = H5_load_global(:H5T_STD_U8BE_g)

const H5T_STD_U8LE = H5_load_global(:H5T_STD_U8LE_g)

const H5T_STD_U16BE = H5_load_global(:H5T_STD_U16BE_g)

const H5T_STD_U16LE = H5_load_global(:H5T_STD_U16LE_g)

const H5T_STD_U32BE = H5_load_global(:H5T_STD_U32BE_g)

const H5T_STD_U32LE = H5_load_global(:H5T_STD_U32LE_g)

const H5T_STD_U64BE = H5_load_global(:H5T_STD_U64BE_g)

const H5T_STD_U64LE = H5_load_global(:H5T_STD_U64LE_g)

const H5T_STD_B8BE = H5_load_global(:H5T_STD_B8BE_g)

const H5T_STD_B8LE = H5_load_global(:H5T_STD_B8LE_g)

const H5T_STD_B16BE = H5_load_global(:H5T_STD_B16BE_g)

const H5T_STD_B16LE = H5_load_global(:H5T_STD_B16LE_g)

const H5T_STD_B32BE = H5_load_global(:H5T_STD_B32BE_g)

const H5T_STD_B32LE = H5_load_global(:H5T_STD_B32LE_g)

const H5T_STD_B64BE = H5_load_global(:H5T_STD_B64BE_g)

const H5T_STD_B64LE = H5_load_global(:H5T_STD_B64LE_g)

const H5T_STD_REF_OBJ = H5_load_global(:H5T_STD_REF_OBJ_g)

const H5T_STD_REF_DSETREG = H5_load_global(:H5T_STD_REF_DSETREG_g)

const H5T_STD_REF = H5_load_global(:H5T_STD_REF_g)

const H5T_UNIX_D32BE = H5_load_global(:H5T_UNIX_D32BE_g)

const H5T_UNIX_D32LE = H5_load_global(:H5T_UNIX_D32LE_g)

const H5T_UNIX_D64BE = H5_load_global(:H5T_UNIX_D64BE_g)

const H5T_UNIX_D64LE = H5_load_global(:H5T_UNIX_D64LE_g)

const H5T_C_S1 = H5_load_global(:H5T_C_S1_g)

const H5T_FORTRAN_S1 = H5_load_global(:H5T_FORTRAN_S1_g)
...
const H5T_VAX_F32 = H5_load_global(:H5T_VAX_F32_g)

const H5T_VAX_F64 = H5_load_global(:H5T_VAX_F64_g)

const H5T_NATIVE_SCHAR = H5_load_global(:H5T_NATIVE_SCHAR_g)

const H5T_NATIVE_UCHAR = H5_load_global(:H5T_NATIVE_UCHAR_g)

const CHAR_MIN = false
...
const H5T_NATIVE_SHORT = H5_load_global(:H5T_NATIVE_SHORT_g)

const H5T_NATIVE_USHORT = H5_load_global(:H5T_NATIVE_USHORT_g)

const H5T_NATIVE_INT = H5_load_global(:H5T_NATIVE_INT_g)

const H5T_NATIVE_UINT = H5_load_global(:H5T_NATIVE_UINT_g)

const H5T_NATIVE_LONG = H5_load_global(:H5T_NATIVE_LONG_g)

const H5T_NATIVE_ULONG = H5_load_global(:H5T_NATIVE_ULONG_g)

const H5T_NATIVE_LLONG = H5_load_global(:H5T_NATIVE_LLONG_g)

const H5T_NATIVE_ULLONG = H5_load_global(:H5T_NATIVE_ULLONG_g)

const H5T_NATIVE_FLOAT = H5_load_global(:H5T_NATIVE_FLOAT_g)

const H5T_NATIVE_DOUBLE = H5_load_global(:H5T_NATIVE_DOUBLE_g)

const H5T_NATIVE_LDOUBLE = H5_load_global(:H5T_NATIVE_LDOUBLE_g)

const H5T_NATIVE_B8 = H5_load_global(:H5T_NATIVE_B8_g)

const H5T_NATIVE_B16 = H5_load_global(:H5T_NATIVE_B16_g)

const H5T_NATIVE_B32 = H5_load_global(:H5T_NATIVE_B32_g)

const H5T_NATIVE_B64 = H5_load_global(:H5T_NATIVE_B64_g)

const H5T_NATIVE_OPAQUE = H5_load_global(:H5T_NATIVE_OPAQUE_g)

const H5T_NATIVE_HADDR = H5_load_global(:H5T_NATIVE_HADDR_g)

const H5T_NATIVE_HSIZE = H5_load_global(:H5T_NATIVE_HSIZE_g)

const H5T_NATIVE_HSSIZE = H5_load_global(:H5T_NATIVE_HSSIZE_g)

const H5T_NATIVE_HERR = H5_load_global(:H5T_NATIVE_HERR_g)

const H5T_NATIVE_HBOOL = H5_load_global(:H5T_NATIVE_HBOOL_g)

const H5T_NATIVE_INT8 = H5_load_global(:H5T_NATIVE_INT8_g)

const H5T_NATIVE_UINT8 = H5_load_global(:H5T_NATIVE_UINT8_g)

const H5T_NATIVE_INT_LEAST8 = H5_load_global(:H5T_NATIVE_INT_LEAST8_g)

const H5T_NATIVE_UINT_LEAST8 = H5_load_global(:H5T_NATIVE_UINT_LEAST8_g)

const H5T_NATIVE_INT_FAST8 = H5_load_global(:H5T_NATIVE_INT_FAST8_g)

const H5T_NATIVE_UINT_FAST8 = H5_load_global(:H5T_NATIVE_UINT_FAST8_g)

const H5T_NATIVE_INT16 = H5_load_global(:H5T_NATIVE_INT16_g)

const H5T_NATIVE_UINT16 = H5_load_global(:H5T_NATIVE_UINT16_g)

const H5T_NATIVE_INT_LEAST16 = H5_load_global(:H5T_NATIVE_INT_LEAST16_g)

const H5T_NATIVE_UINT_LEAST16 = H5_load_global(:H5T_NATIVE_UINT_LEAST16_g)

const H5T_NATIVE_INT_FAST16 = H5_load_global(:H5T_NATIVE_INT_FAST16_g)

const H5T_NATIVE_UINT_FAST16 = H5_load_global(:H5T_NATIVE_UINT_FAST16_g)

const H5T_NATIVE_INT32 = H5_load_global(:H5T_NATIVE_INT32_g)

const H5T_NATIVE_UINT32 = H5_load_global(:H5T_NATIVE_UINT32_g)

const H5T_NATIVE_INT_LEAST32 = H5_load_global(:H5T_NATIVE_INT_LEAST32_g)

const H5T_NATIVE_UINT_LEAST32 = H5_load_global(:H5T_NATIVE_UINT_LEAST32_g)

const H5T_NATIVE_INT_FAST32 = H5_load_global(:H5T_NATIVE_INT_FAST32_g)

const H5T_NATIVE_UINT_FAST32 = H5_load_global(:H5T_NATIVE_UINT_FAST32_g)

const H5T_NATIVE_INT64 = H5_load_global(:H5T_NATIVE_INT64_g)

const H5T_NATIVE_UINT64 = H5_load_global(:H5T_NATIVE_UINT64_g)

const H5T_NATIVE_INT_LEAST64 = H5_load_global(:H5T_NATIVE_INT_LEAST64_g)

const H5T_NATIVE_UINT_LEAST64 = H5_load_global(:H5T_NATIVE_UINT_LEAST64_g)

const H5T_NATIVE_INT_FAST64 = H5_load_global(:H5T_NATIVE_INT_FAST64_g)

const H5T_NATIVE_UINT_FAST64 = H5_load_global(:H5T_NATIVE_UINT_FAST64_g)

const H5L_MAX_LINK_NAME_LEN = Int32(-1)
...
const H5O_TOKEN_UNDEF = H5_load_global(:H5O_TOKEN_UNDEF_g)
...
const H5D_CHUNK_CACHE_NSLOTS_DEFAULT = Cssize_t(- 1)

const H5D_CHUNK_CACHE_NBYTES_DEFAULT = Cssize_t(- 1)
...
const H5E_ERR_CLS = H5_load_global(:H5E_ERR_CLS_g)

const H5E_FUNC = H5_load_global(:H5E_FUNC_g)

const H5E_FILE = H5_load_global(:H5E_FILE_g)

const H5E_VOL = H5_load_global(:H5E_VOL_g)

const H5E_SOHM = H5_load_global(:H5E_SOHM_g)

const H5E_SYM = H5_load_global(:H5E_SYM_g)

const H5E_PLUGIN = H5_load_global(:H5E_PLUGIN_g)

const H5E_VFL = H5_load_global(:H5E_VFL_g)

const H5E_INTERNAL = H5_load_global(:H5E_INTERNAL_g)

const H5E_BTREE = H5_load_global(:H5E_BTREE_g)

const H5E_REFERENCE = H5_load_global(:H5E_REFERENCE_g)

const H5E_DATASPACE = H5_load_global(:H5E_DATASPACE_g)

const H5E_RESOURCE = H5_load_global(:H5E_RESOURCE_g)

const H5E_RS = H5_load_global(:H5E_RS_g)

const H5E_FARRAY = H5_load_global(:H5E_FARRAY_g)

const H5E_HEAP = H5_load_global(:H5E_HEAP_g)

const H5E_MAP = H5_load_global(:H5E_MAP_g)

const H5E_ATTR = H5_load_global(:H5E_ATTR_g)

const H5E_IO = H5_load_global(:H5E_IO_g)

const H5E_EFL = H5_load_global(:H5E_EFL_g)

const H5E_TST = H5_load_global(:H5E_TST_g)

const H5E_LIB = H5_load_global(:H5E_LIB_g)

const H5E_PAGEBUF = H5_load_global(:H5E_PAGEBUF_g)

const H5E_FSPACE = H5_load_global(:H5E_FSPACE_g)

const H5E_DATASET = H5_load_global(:H5E_DATASET_g)

const H5E_STORAGE = H5_load_global(:H5E_STORAGE_g)

const H5E_LINK = H5_load_global(:H5E_LINK_g)

const H5E_PLIST = H5_load_global(:H5E_PLIST_g)

const H5E_DATATYPE = H5_load_global(:H5E_DATATYPE_g)

const H5E_OHDR = H5_load_global(:H5E_OHDR_g)

const H5E_ATOM = H5_load_global(:H5E_ATOM_g)

const H5E_NONE_MAJOR = H5_load_global(:H5E_NONE_MAJOR_g)

const H5E_SLIST = H5_load_global(:H5E_SLIST_g)

const H5E_ARGS = H5_load_global(:H5E_ARGS_g)

const H5E_CONTEXT = H5_load_global(:H5E_CONTEXT_g)

const H5E_EARRAY = H5_load_global(:H5E_EARRAY_g)

const H5E_PLINE = H5_load_global(:H5E_PLINE_g)

const H5E_ERROR = H5_load_global(:H5E_ERROR_g)

const H5E_CACHE = H5_load_global(:H5E_CACHE_g)

const H5E_SEEKERROR = H5_load_global(:H5E_SEEKERROR_g)

const H5E_READERROR = H5_load_global(:H5E_READERROR_g)

const H5E_WRITEERROR = H5_load_global(:H5E_WRITEERROR_g)

const H5E_CLOSEERROR = H5_load_global(:H5E_CLOSEERROR_g)

const H5E_OVERFLOW = H5_load_global(:H5E_OVERFLOW_g)

const H5E_FCNTL = H5_load_global(:H5E_FCNTL_g)

const H5E_NOSPACE = H5_load_global(:H5E_NOSPACE_g)

const H5E_CANTALLOC = H5_load_global(:H5E_CANTALLOC_g)

const H5E_CANTCOPY = H5_load_global(:H5E_CANTCOPY_g)

const H5E_CANTFREE = H5_load_global(:H5E_CANTFREE_g)

const H5E_ALREADYEXISTS = H5_load_global(:H5E_ALREADYEXISTS_g)

const H5E_CANTLOCK = H5_load_global(:H5E_CANTLOCK_g)

const H5E_CANTUNLOCK = H5_load_global(:H5E_CANTUNLOCK_g)

const H5E_CANTGC = H5_load_global(:H5E_CANTGC_g)

const H5E_CANTGETSIZE = H5_load_global(:H5E_CANTGETSIZE_g)

const H5E_OBJOPEN = H5_load_global(:H5E_OBJOPEN_g)

const H5E_CANTRESTORE = H5_load_global(:H5E_CANTRESTORE_g)

const H5E_CANTCOMPUTE = H5_load_global(:H5E_CANTCOMPUTE_g)

const H5E_CANTEXTEND = H5_load_global(:H5E_CANTEXTEND_g)

const H5E_CANTATTACH = H5_load_global(:H5E_CANTATTACH_g)

const H5E_CANTUPDATE = H5_load_global(:H5E_CANTUPDATE_g)

const H5E_CANTOPERATE = H5_load_global(:H5E_CANTOPERATE_g)

const H5E_CANTINIT = H5_load_global(:H5E_CANTINIT_g)

const H5E_ALREADYINIT = H5_load_global(:H5E_ALREADYINIT_g)

const H5E_CANTRELEASE = H5_load_global(:H5E_CANTRELEASE_g)

const H5E_CANTGET = H5_load_global(:H5E_CANTGET_g)

const H5E_CANTSET = H5_load_global(:H5E_CANTSET_g)

const H5E_DUPCLASS = H5_load_global(:H5E_DUPCLASS_g)

const H5E_SETDISALLOWED = H5_load_global(:H5E_SETDISALLOWED_g)

const H5E_CANTMERGE = H5_load_global(:H5E_CANTMERGE_g)

const H5E_CANTREVIVE = H5_load_global(:H5E_CANTREVIVE_g)

const H5E_CANTSHRINK = H5_load_global(:H5E_CANTSHRINK_g)

const H5E_LINKCOUNT = H5_load_global(:H5E_LINKCOUNT_g)

const H5E_VERSION = H5_load_global(:H5E_VERSION_g)

const H5E_ALIGNMENT = H5_load_global(:H5E_ALIGNMENT_g)

const H5E_BADMESG = H5_load_global(:H5E_BADMESG_g)

const H5E_CANTDELETE = H5_load_global(:H5E_CANTDELETE_g)

const H5E_BADITER = H5_load_global(:H5E_BADITER_g)

const H5E_CANTPACK = H5_load_global(:H5E_CANTPACK_g)

const H5E_CANTRESET = H5_load_global(:H5E_CANTRESET_g)

const H5E_CANTRENAME = H5_load_global(:H5E_CANTRENAME_g)

const H5E_SYSERRSTR = H5_load_global(:H5E_SYSERRSTR_g)

const H5E_NOFILTER = H5_load_global(:H5E_NOFILTER_g)

const H5E_CALLBACK = H5_load_global(:H5E_CALLBACK_g)

const H5E_CANAPPLY = H5_load_global(:H5E_CANAPPLY_g)

const H5E_SETLOCAL = H5_load_global(:H5E_SETLOCAL_g)

const H5E_NOENCODER = H5_load_global(:H5E_NOENCODER_g)

const H5E_CANTFILTER = H5_load_global(:H5E_CANTFILTER_g)

const H5E_CANTOPENOBJ = H5_load_global(:H5E_CANTOPENOBJ_g)

const H5E_CANTCLOSEOBJ = H5_load_global(:H5E_CANTCLOSEOBJ_g)

const H5E_COMPLEN = H5_load_global(:H5E_COMPLEN_g)

const H5E_PATH = H5_load_global(:H5E_PATH_g)

const H5E_NONE_MINOR = H5_load_global(:H5E_NONE_MINOR_g)

const H5E_OPENERROR = H5_load_global(:H5E_OPENERROR_g)

const H5E_FILEEXISTS = H5_load_global(:H5E_FILEEXISTS_g)

const H5E_FILEOPEN = H5_load_global(:H5E_FILEOPEN_g)

const H5E_CANTCREATE = H5_load_global(:H5E_CANTCREATE_g)

const H5E_CANTOPENFILE = H5_load_global(:H5E_CANTOPENFILE_g)

const H5E_CANTCLOSEFILE = H5_load_global(:H5E_CANTCLOSEFILE_g)

const H5E_NOTHDF5 = H5_load_global(:H5E_NOTHDF5_g)

const H5E_BADFILE = H5_load_global(:H5E_BADFILE_g)

const H5E_TRUNCATED = H5_load_global(:H5E_TRUNCATED_g)

const H5E_MOUNT = H5_load_global(:H5E_MOUNT_g)

const H5E_CANTDELETEFILE = H5_load_global(:H5E_CANTDELETEFILE_g)

const H5E_CANTLOCKFILE = H5_load_global(:H5E_CANTLOCKFILE_g)

const H5E_CANTUNLOCKFILE = H5_load_global(:H5E_CANTUNLOCKFILE_g)

const H5E_BADATOM = H5_load_global(:H5E_BADATOM_g)

const H5E_BADGROUP = H5_load_global(:H5E_BADGROUP_g)

const H5E_CANTREGISTER = H5_load_global(:H5E_CANTREGISTER_g)

const H5E_CANTINC = H5_load_global(:H5E_CANTINC_g)

const H5E_CANTDEC = H5_load_global(:H5E_CANTDEC_g)

const H5E_NOIDS = H5_load_global(:H5E_NOIDS_g)

const H5E_CANTFLUSH = H5_load_global(:H5E_CANTFLUSH_g)

const H5E_CANTUNSERIALIZE = H5_load_global(:H5E_CANTUNSERIALIZE_g)

const H5E_CANTSERIALIZE = H5_load_global(:H5E_CANTSERIALIZE_g)

const H5E_CANTTAG = H5_load_global(:H5E_CANTTAG_g)

const H5E_CANTLOAD = H5_load_global(:H5E_CANTLOAD_g)

const H5E_PROTECT = H5_load_global(:H5E_PROTECT_g)

const H5E_NOTCACHED = H5_load_global(:H5E_NOTCACHED_g)

const H5E_SYSTEM = H5_load_global(:H5E_SYSTEM_g)

const H5E_CANTINS = H5_load_global(:H5E_CANTINS_g)

const H5E_CANTPROTECT = H5_load_global(:H5E_CANTPROTECT_g)

const H5E_CANTUNPROTECT = H5_load_global(:H5E_CANTUNPROTECT_g)

const H5E_CANTPIN = H5_load_global(:H5E_CANTPIN_g)

const H5E_CANTUNPIN = H5_load_global(:H5E_CANTUNPIN_g)

const H5E_CANTMARKDIRTY = H5_load_global(:H5E_CANTMARKDIRTY_g)

const H5E_CANTMARKCLEAN = H5_load_global(:H5E_CANTMARKCLEAN_g)

const H5E_CANTMARKUNSERIALIZED = H5_load_global(:H5E_CANTMARKUNSERIALIZED_g)

const H5E_CANTMARKSERIALIZED = H5_load_global(:H5E_CANTMARKSERIALIZED_g)

const H5E_CANTDIRTY = H5_load_global(:H5E_CANTDIRTY_g)

const H5E_CANTCLEAN = H5_load_global(:H5E_CANTCLEAN_g)

const H5E_CANTEXPUNGE = H5_load_global(:H5E_CANTEXPUNGE_g)

const H5E_CANTRESIZE = H5_load_global(:H5E_CANTRESIZE_g)

const H5E_CANTDEPEND = H5_load_global(:H5E_CANTDEPEND_g)

const H5E_CANTUNDEPEND = H5_load_global(:H5E_CANTUNDEPEND_g)

const H5E_CANTNOTIFY = H5_load_global(:H5E_CANTNOTIFY_g)

const H5E_LOGGING = H5_load_global(:H5E_LOGGING_g)

const H5E_CANTCORK = H5_load_global(:H5E_CANTCORK_g)

const H5E_CANTUNCORK = H5_load_global(:H5E_CANTUNCORK_g)

const H5E_TRAVERSE = H5_load_global(:H5E_TRAVERSE_g)

const H5E_NLINKS = H5_load_global(:H5E_NLINKS_g)

const H5E_NOTREGISTERED = H5_load_global(:H5E_NOTREGISTERED_g)

const H5E_CANTMOVE = H5_load_global(:H5E_CANTMOVE_g)

const H5E_CANTSORT = H5_load_global(:H5E_CANTSORT_g)

const H5E_MPI = H5_load_global(:H5E_MPI_g)

const H5E_MPIERRSTR = H5_load_global(:H5E_MPIERRSTR_g)

const H5E_CANTRECV = H5_load_global(:H5E_CANTRECV_g)

const H5E_CANTGATHER = H5_load_global(:H5E_CANTGATHER_g)

const H5E_NO_INDEPENDENT = H5_load_global(:H5E_NO_INDEPENDENT_g)

const H5E_CANTCLIP = H5_load_global(:H5E_CANTCLIP_g)

const H5E_CANTCOUNT = H5_load_global(:H5E_CANTCOUNT_g)

const H5E_CANTSELECT = H5_load_global(:H5E_CANTSELECT_g)

const H5E_CANTNEXT = H5_load_global(:H5E_CANTNEXT_g)

const H5E_BADSELECT = H5_load_global(:H5E_BADSELECT_g)

const H5E_CANTCOMPARE = H5_load_global(:H5E_CANTCOMPARE_g)

const H5E_INCONSISTENTSTATE = H5_load_global(:H5E_INCONSISTENTSTATE_g)

const H5E_CANTAPPEND = H5_load_global(:H5E_CANTAPPEND_g)

const H5E_UNINITIALIZED = H5_load_global(:H5E_UNINITIALIZED_g)

const H5E_UNSUPPORTED = H5_load_global(:H5E_UNSUPPORTED_g)

const H5E_BADTYPE = H5_load_global(:H5E_BADTYPE_g)

const H5E_BADRANGE = H5_load_global(:H5E_BADRANGE_g)

const H5E_BADVALUE = H5_load_global(:H5E_BADVALUE_g)

const H5E_NOTFOUND = H5_load_global(:H5E_NOTFOUND_g)

const H5E_EXISTS = H5_load_global(:H5E_EXISTS_g)

const H5E_CANTENCODE = H5_load_global(:H5E_CANTENCODE_g)

const H5E_CANTDECODE = H5_load_global(:H5E_CANTDECODE_g)

const H5E_CANTSPLIT = H5_load_global(:H5E_CANTSPLIT_g)

const H5E_CANTREDISTRIBUTE = H5_load_global(:H5E_CANTREDISTRIBUTE_g)

const H5E_CANTSWAP = H5_load_global(:H5E_CANTSWAP_g)

const H5E_CANTINSERT = H5_load_global(:H5E_CANTINSERT_g)

const H5E_CANTLIST = H5_load_global(:H5E_CANTLIST_g)

const H5E_CANTMODIFY = H5_load_global(:H5E_CANTMODIFY_g)

const H5E_CANTREMOVE = H5_load_global(:H5E_CANTREMOVE_g)

const H5E_CANTCONVERT = H5_load_global(:H5E_CANTCONVERT_g)

const H5E_BADSIZE = H5_load_global(:H5E_BADSIZE_g)
...
const H5F_UNLIMITED = Clonglong(Clong(-1))
...
const H5P_ROOT = H5_load_global(:H5P_CLS_ROOT_ID_g)

const H5P_OBJECT_CREATE = H5_load_global(:H5P_CLS_OBJECT_CREATE_ID_g)

const H5P_FILE_CREATE = H5_load_global(:H5P_CLS_FILE_CREATE_ID_g)

const H5P_FILE_ACCESS = H5_load_global(:H5P_CLS_FILE_ACCESS_ID_g)

const H5P_DATASET_CREATE = H5_load_global(:H5P_CLS_DATASET_CREATE_ID_g)

const H5P_DATASET_ACCESS = H5_load_global(:H5P_CLS_DATASET_ACCESS_ID_g)

const H5P_DATASET_XFER = H5_load_global(:H5P_CLS_DATASET_XFER_ID_g)

const H5P_FILE_MOUNT = H5_load_global(:H5P_CLS_FILE_MOUNT_ID_g)

const H5P_GROUP_CREATE = H5_load_global(:H5P_CLS_GROUP_CREATE_ID_g)

const H5P_GROUP_ACCESS = H5_load_global(:H5P_CLS_GROUP_ACCESS_ID_g)

const H5P_DATATYPE_CREATE = H5_load_global(:H5P_CLS_DATATYPE_CREATE_ID_g)

const H5P_DATATYPE_ACCESS = H5_load_global(:H5P_CLS_DATATYPE_ACCESS_ID_g)

const H5P_MAP_CREATE = H5_load_global(:H5P_CLS_MAP_CREATE_ID_g)

const H5P_MAP_ACCESS = H5_load_global(:H5P_CLS_MAP_ACCESS_ID_g)

const H5P_STRING_CREATE = H5_load_global(:H5P_CLS_STRING_CREATE_ID_g)

const H5P_ATTRIBUTE_CREATE = H5_load_global(:H5P_CLS_ATTRIBUTE_CREATE_ID_g)

const H5P_ATTRIBUTE_ACCESS = H5_load_global(:H5P_CLS_ATTRIBUTE_ACCESS_ID_g)

const H5P_OBJECT_COPY = H5_load_global(:H5P_CLS_OBJECT_COPY_ID_g)

const H5P_LINK_CREATE = H5_load_global(:H5P_CLS_LINK_CREATE_ID_g)

const H5P_LINK_ACCESS = H5_load_global(:H5P_CLS_LINK_ACCESS_ID_g)

const H5P_VOL_INITIALIZE = H5_load_global(:H5P_CLS_VOL_INITIALIZE_ID_g)

const H5P_REFERENCE_ACCESS = H5_load_global(:H5P_CLS_REFERENCE_ACCESS_ID_g)

const H5P_FILE_CREATE_DEFAULT = H5_load_global(:H5P_LST_FILE_CREATE_ID_g)

const H5P_FILE_ACCESS_DEFAULT = H5_load_global(:H5P_LST_FILE_ACCESS_ID_g)

const H5P_DATASET_CREATE_DEFAULT = H5_load_global(:H5P_LST_DATASET_CREATE_ID_g)

const H5P_DATASET_ACCESS_DEFAULT = H5_load_global(:H5P_LST_DATASET_ACCESS_ID_g)

const H5P_DATASET_XFER_DEFAULT = H5_load_global(:H5P_LST_DATASET_XFER_ID_g)

const H5P_FILE_MOUNT_DEFAULT = H5_load_global(:H5P_LST_FILE_MOUNT_ID_g)

const H5P_GROUP_CREATE_DEFAULT = H5_load_global(:H5P_LST_GROUP_CREATE_ID_g)

const H5P_GROUP_ACCESS_DEFAULT = H5_load_global(:H5P_LST_GROUP_ACCESS_ID_g)

const H5P_DATATYPE_CREATE_DEFAULT = H5_load_global(:H5P_LST_DATATYPE_CREATE_ID_g)

const H5P_DATATYPE_ACCESS_DEFAULT = H5_load_global(:H5P_LST_DATATYPE_ACCESS_ID_g)

const H5P_MAP_CREATE_DEFAULT = H5_load_global(:H5P_LST_MAP_CREATE_ID_g)

const H5P_MAP_ACCESS_DEFAULT = H5_load_global(:H5P_LST_MAP_ACCESS_ID_g)

const H5P_ATTRIBUTE_CREATE_DEFAULT = H5_load_global(:H5P_LST_ATTRIBUTE_CREATE_ID_g)

const H5P_ATTRIBUTE_ACCESS_DEFAULT = H5_load_global(:H5P_LST_ATTRIBUTE_ACCESS_ID_g)

const H5P_OBJECT_COPY_DEFAULT = H5_load_global(:H5P_LST_OBJECT_COPY_ID_g)

const H5P_LINK_CREATE_DEFAULT = H5_load_global(:H5P_LST_LINK_CREATE_ID_g)

const H5P_LINK_ACCESS_DEFAULT = H5_load_global(:H5P_LST_LINK_ACCESS_ID_g)

const H5P_VOL_INITIALIZE_DEFAULT = H5_load_global(:H5P_LST_VOL_INITIALIZE_ID_g)

const H5P_REFERENCE_ACCESS_DEFAULT = H5_load_global(:H5P_LST_REFERENCE_ACCESS_ID_g)
